...
document.onreadystatechange = function(){
    drawGauge('temGauge','温湿度传感器','温度gauge',23,'温度','℃');
    drawGauge('humiGauge','温湿度传感器','湿度gauge',45,'湿度','%');
    var timestamp = Date.parse(new Date());
    var startTs=(timestamp / 1000 - 24 * 60 * 60 ) * 1000;
    var endTs=timestamp;
    fetchLineData('1bfca680-9c75-11e9-9dcf-b55ae51a103e','temperature',startTs,endTs)
}
function drawGauge(container, titleName, seriesName, seriesData,dataName,dataUnit){
    var myHumiChart = echarts.init(document.getElementById(container));
    var humiOption = {
        title: {
            text: titleName, 
            textStyle:{
    ...
    myHumiChart.setOption(humiOption);
}

function drawLine(deviceId) {
    var myChart = echarts.init(document.getElementById('temHumiLine'));
    //startTs,endTs设置
    var timestamp = Date.parse(new Date());
    var startTs=(timestamp / 1000 - 24 * 60 * 60 ) * 1000;
    var endTs=timestamp;
    var res=fetchLineData(deviceId,'temperature',startTs,endTs);
    var xData=res['x'];
    var seriesDataA=res['y'];
    res=fetchLineData(deviceId,'humidity',startTs,endTs);
    xData=[];
    var seriesDataB=res['y'];
    myOption = {
        toolbox: { 
            show: true,
            feature: {
...
function fetchLineData(deviceId,dataType,startTs,endTs){
    console.log("starthjjjjjjjjjjjjjjjj");
    axios({
        method:'get',
        url:'https://47.105.120.203:30080/api/v1/deviceaccess/data/alldata',
...
function fetchLinePointData(resData,dataType){
    ...
    for ( var i = 0; i <resData.length; i++){
        if(resData[i]['key']===dataType){
            timeArray.push(resData[i]['ts']);
            valueArray.push(resData[i]['value']);
        }
    }
