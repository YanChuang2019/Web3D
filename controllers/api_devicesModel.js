...
module.exports = {
    ...
    'GET /api/dModel/getSitedModel/:siteId': async (ctx, next) => {  //通过站点Id获取当前站点下的所有模型
        var siteId = ctx.params.siteId;
        var res = await dModel.getSitedModel(siteId);  //数据库操作，用来获取模型   
        ctx.rest({
            dModels: res
        });
    },
