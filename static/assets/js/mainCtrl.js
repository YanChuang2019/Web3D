...
//点击添加模型显示框
    ...
    $scope.submitModel = function(){
        ...
        jQuery.ajax({
            ...
            data: {
                ...
                "name": $scope.deviceInfo.nickname,     // nickname 替代 name
    ...
    var saveAllPosition = function(){//更新全部模型的位置信息
        for(let i = scene.children.length-1;i>=0;i--){
            if(scene.children[i].type === "Mesh"){
                downIntersected = scene.children[i];
                ...
                $.ajax({
                    url:'/api/dModel/dModelLocation/'+ 83,
                    type:'PUT',
                    data:{
    ...
    $scope._updatePosition = function() {//更新新建模型的位置
        for(let i = scene.children.length-1;i>=0;i--){
            if(scene.children[i].type === "Mesh"){
                var deviceid = scene.children[i].deviceId;
                downIntersected = scene.children[i];
                var target_p = new THREE.Vector3();
                downIntersected.getWorldPosition(target_p);
                var target_s = new THREE.Vector3();
                ...
                console.log(newLocation);
                $.ajax({
                    url:'/api/dModel/dModelLocation/'+ deviceid,
                    type:'PUT',
                    data:{
                        "location":JSON.stringify(newLocation)
                    },
...
$scope.updateFile = function(){
    function uploadProgress(evt) {  
      ...
      else if(fileType != "drc"&&fileType !="obj"&&fileType !="max"&&fileType !="glft" &&fileType !="stl") 
      {
           alert("请上传正确格式文件")
      }
      else
...
$scope.uploadModel = function(){//添加新的模型
    ...
    //alert($scope.deviceInfo.id)
    jQuery.ajax({
        url: '/api/dModel/createModel/'+tenantId+'/'+siteId,
        dataType: 'json',
        method: 'POST',
        data: {
            "deviceId": 28,//$scope.deviceInfo.id,
            "location": JSON.stringify(JSONBody),
            "name": "pipeline17",//$scope.deviceInfo.name,
